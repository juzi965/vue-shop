    ...
    <div class="shop-content">
      <el-row :gutter="30">
        <el-col
          ...
          :lg="4"
          :xl="4"
          v-for="clothing in clothingInfos"
          :key="clothing.id"
        >
          <el-card :body-style="{ padding: '0px' }" class="shop-card">
            <img
              @click="getClothingInfo(clothing.id)"
              class="image"
              :src="
...
export default {
  ...
  methods: {
    initData() {
      this.$http
        .get('/clothing/category/' + this.$route.params.name)
        .then(res => {
